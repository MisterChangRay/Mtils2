/**
 *
 *  BSD 3-Clause License https://github.com/MisterChangRay/Mtils2/blob/master/LICENSE
 *   Author zhangrui
 */
;'use strict';
...
;(function(){
    ...
    function Mtils() {
		var Mtils = {
		    ...
		    validation: {
		        ...
		        isBankCard: function(str_cardNo) {
		            ...
		            var lastNum = str_cardNo.substr(str_cardNo.length - 1, 1);
		            var first15Num = str_cardNo.substr(0, str_cardNo.length - 1);
		    ...
		    extention: {
		        ...
		        now: function(format) {
		            ...
		            if (format) return Mtils.extention.formatDate(timestamp, format);
		            return timestamp
		        },
		    ...
		    utils: {
		            ...
		            searchs.forEach(function(search) {
		                ...
		                result = result.concat(arr.filter(function(item) {
		                    if (typeof(item) === typeof(search)) {
		                        ...
		                        } else if (item === search) return true
		                    }
		                }))
		            });
		            return result
		        ...
		        makeMap: function(str, defaultVal) {
		            var obj = {},
		                items = Mtils.extention.trim(str).split(","),
		                i;
		            for (i = 0; i < items.length; i++) obj[items[i]] = defaultVal;
		            return obj
		        },
		        ...
		        decimalConversion: function(number, from, to) {
		            var origin = Mtils.constant.BASE_DECIMAL,
		                tmp, decimal = 0,
		                result = "";
		            ...
		            if (Mtils.utils.isString(str_data)) {
		                ...
		                if (str_data) {
		                    for (var i = 0; i < str_data.length; i++) {
		                        if (str_data.substring(i, i + 1).match(/[^\x00-\xff]/ig) != null) Zhlength += 1;
		                        else Enlength += 1
		                    }
		                }
		...
		global.ChainCallManager = Mtils.ChainCallManager = Mtils.utils.ChainCallManager;
		global.isDefined = Mtils.isDefined = Mtils.utils.isDefined;
		global.isEmpty = Mtils.isEmpty = Mtils.utils.isEmpty;
		global.strToJson = Mtils.strToJson = Mtils.extention.strToJson;
